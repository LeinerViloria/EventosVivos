...
### Integración con PrimeNG mediante el puente de compatibilidad

La directiva nativa de Signal Forms para enlazar un control es `[formField]`. Al enlazarla, el chequeo de tipos de plantilla de Angular trata al elemento anfitrión como si implementara el contrato `FormUiControl`/`FormValueControl` de Signal Forms. Los componentes de PrimeNG 21 son anteriores a Signal Forms y se construyen sobre `ControlValueAccessor`, el mecanismo de Reactive Forms; además exponen inputs cuyos nombres coinciden con los de ese contrato pero con tipos incompatibles (por ejemplo, `pattern`). Esa colisión hace que `[formField]` no compile sobre los componentes de PrimeNG.

La solución no es abandonar Signal Forms, sino usar el puente oficial `@angular/forms/signals/compat`. Su clase `SignalFormControl` extiende `AbstractControl`: cada campo se declara con las reglas de Signal Forms (`required`, `minLength`, `maxLength`, validadores personalizados), se agrupa en un `FormGroup` y se enlaza en la plantilla con `formControlName`, la directiva de Reactive Forms que PrimeNG sí soporta de forma nativa a través de `ControlValueAccessor`. Así se conserva la autoría de la validación en Signal Forms y se mantiene la regla de no usar HTML crudo, componiendo con los componentes de la librería. Cuando PrimeNG publique una versión alineada con Angular 22 y el contrato de controles de Signal Forms, se podrá volver al enlace directo con `[formField]`.
