...
En el canal de notificaciones en tiempo real mediante Server-Sent Events existe una consideración particular. La API `EventSource` del navegador no permite enviar el header de autorización, por lo que el token de identidad se transmite por medio de la cadena de consulta de la URL. Se descartó el uso de una cookie por considerarse sobreingeniería para el alcance actual. Esta opción se asume de forma consciente, apoyada en que el token tiene un tiempo de vida corto, está firmado, es solo de identidad y corresponde a una sesión revocable contra Redis; el costo conocido es que queda visible en la URL, y la cookie httpOnly queda como evolución de seguridad futura.
